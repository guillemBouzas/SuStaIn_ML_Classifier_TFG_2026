...
df4 = pd.read_csv('supersynth_vols.csv', sep=',')
df4["total intracranial"] = df4.select_dtypes(include="number").sum(axis=1) # aconseguim el total intracranial
df4 = df4[['total intracranial','subject']]
...
# Unir dataframes
df_total = pd.merge(df1, dfNextbrain, on='subject')
...
cols_to_correct = X.columns.drop(['age', 'abeta_pos', 'total intracranial'])
...
X.drop(columns=['age', 'abeta_pos', 'total intracranial'], inplace=True)
...
# Llistes per guardar els resultats de cada "fold"
resultados_acc = []
resultados_sens = []
resultados_espec = []
resultados_prec = []
