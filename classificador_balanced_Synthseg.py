# -*- coding: utf-8 -*-
"""
Created on Wed Mar 18 11:24:07 2026
...
df2 = pd.read_csv('A4_synthseg_vols.csv', sep = ',')


# Unir dataframes
df_total = pd.merge(df1, df2, left_on='BID', right_on='subject')


# Eliminem les columnes que no necessitem
df_total.drop(columns = ['PTGENDER', 'PTRACE','APOE_E4_COUNT', 'PTEDUCAT', 'Unnamed: 0', 'session'], inplace = True, errors='ignore')
...
cols_to_correct = X.columns.drop(['PTAGE', 'total intracranial', 'SCORE'])
...
X.drop(columns = ['total intracranial', 'PTAGE', 'SCORE'], inplace = True)
...
from sklearn.utils import resample # Per aconseguir que a cada fold hi hagi el mateix nombre de positius que de negatius
