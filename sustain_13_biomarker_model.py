...
df5 = pandas.read_csv('nextbrain_only_CA_grouped.csv', sep=',')
...
# -- Anem a gestionar les regions de l'hipocamp

# -- Ajuntarem el dentate gyrus en una sola variable
cols_dentate = df5.filter(like='dentate_gyrus').columns
df5['DG'] = df5[cols_dentate].sum(axis=1)
df5 = df5.drop(columns=cols_dentate)
# --

# -- El mateix amb el subiculum
cols_subiculum = df5.filter(like='subiculum').columns
df5['subiculum'] = df5[cols_subiculum].sum(axis=1)
df5 = df5.drop(columns=cols_subiculum)
# --

# -- Ajuntem els df
data = pandas.merge(data, df5, on= 'subject', how='inner')
# --
...
data = data[['SCORE',
             ...
             'total_TAU',
             'cerebral cortex',
             'amygdalohippocampal_transition_area',
             'fimbria',
             'alveus',
             'CA1',
             'CA2',
...
# Limitem els valors al rang [-5, 5] perquè coincideixin Z_vals y Z_max
...
plt.clf() # Per netejar les figures
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
for i, subtype in enumerate(subtypes):
    ...
    if len(x_vals) > 1 and y_vals.std() > 0:
        ...
        # Escrivim els resultats al gràfic
        ax.text(0.05, 0.95 - (i * 0.05), text_label,
                transform=ax.transAxes,
                fontweight='bold',
                bbox=dict(facecolor='white', alpha=0.5, edgecolor='none'))
...
# Boxplots
vars_to_plot = ['Composite_Summary', 'AGE_REAL', 'total_TAU_real', 'amygdala', 'cerebral cortex']
